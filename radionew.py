...
class RadioGUI:
    ...
    def build_layout(self) -> None:
        ...
        ttk.Label(right,text='Discord Webhook URL',style='Body.TLabel').pack(anchor='w'); ttk.Entry(right,textvariable=self.webhook_var,show='•').pack(fill='x',pady=(0,8)); ttk.Label(right,text='Discord Bot Token',style='Body.TLabel').pack(anchor='w'); ttk.Entry(right,textvariable=self.discord_token_var,show='•').pack(fill='x',pady=(0,8)); ttk.Button(right,text='Save Discord Settings',command=self.apply_settings).pack(fill='x',pady=(0,6)); ttk.Button(right,text='Start Discord Bot',command=self.start_discord_bot).pack(fill='x',pady=(0,10)); ttk.Label(right,text='Priority / Request Queue',style='Value.TLabel').pack(anchor='w'); self.queue_box=tk.Listbox(right,bg='#08111f',fg='white',borderwidth=0); self.queue_box.pack(fill='both',expand=True); ttk.Label(right,text='Automation modules ready: scheduler, REST dashboard, Discord bot, SQLite stats, recording, streaming outputs, themes, diagnostics.',style='Body.TLabel',wraplength=300).pack(anchor='w',pady=10)
    def apply_settings(self) -> None:
        self.config.audio.volume=float(self.volume_var.get()); self.config.audio.crossfade_seconds=float(self.crossfade_var.get()); self.config.audio.shuffle=bool(self.shuffle_var.get()); self.config.audio.loop=bool(self.loop_var.get()); self.config.audio.monitor_local=bool(self.monitor_var.get()); self.config.audio.broadcast_processing=bool(self.processing_var.get()); self.config.discord_webhook_url=self.webhook_var.get().strip(); self.config.discord_token=self.discord_token_var.get().strip(); self.webhook.webhook_url=self.config.discord_webhook_url; self.config.save()
    def refresh_library(self) -> None:
        for path in self.library.scan(): self.db.upsert_track(self.metadata.read(path))
